...
// Открытие модального окна складчины
...
document.getElementById('reserve-form').onsubmit = async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert('⚠️ ' + (data.error || 'Ошибка бронирования'));
    }
  } catch (err) {
...
document.getElementById('toggle-admin').onclick = () => {
  ...
  if (!panel.hidden && !isAdmin) {
    ...
    if (key) {
      // Проверка будет на сервере, здесь просто показываем форму
      isAdmin = true;
    } else {
      panel.hidden = true;
    }
...
document.getElementById('add-gift-form').onsubmit = async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert('⚠️ ' + (data.error || 'Ошибка добавления'));
    }
  } catch (err) {
    alert('❌ Ошибка соединения');
  }
...
document.addEventListener('DOMContentLoaded', () => {
  loadGifts();
  // Автообновление каждые 30 сек (real-time эффект без WebSocket)
